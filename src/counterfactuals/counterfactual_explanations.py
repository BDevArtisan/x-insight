...
def generate_counterfactual_optimization(
    ...
    if target_cluster is None:
        ...
        possible_targets = [c for c in unique_clusters if c != current_cluster and c >= 0]
    ...
    if target_cluster not in unique_labels:
        return None
    ...
    if centers is None or len(centers) == 0:
        ...
        for c in unique_labels:
            ...
            if len(cluster_points) > 0:
                ...
                centers.append(np.zeros(X_arr.shape[1]))
        centers = np.array(centers)
    
    if centers is None or len(centers) == 0:
        return None
    
    if target_cluster >= len(centers):
        cluster_mapping = {c: i for i, c in enumerate(unique_labels)}
        if target_cluster not in cluster_mapping:
            return None
...
def generate_diverse_counterfactuals(
    ...
    for i in range(n_counterfactuals * 2):
...
def format_counterfactual_explanation(counterfactual, feature_names=None):
    if not counterfactual or not counterfactual.get('success', False):
    ...
    if not explanation_parts:
        return "No significant changes required"
...
def plot_diverse_counterfactuals(counterfactuals, feature_names=None, top_n=5):
    ...
    top_features = top_features[top_features < len(feature_names)]
    ...
    for i, cf in enumerate(counterfactuals):
        if 'changes' not in cf or 'distance' not in cf:
            continue
...
def evaluate_counterfactual_quality(counterfactual, constraints=None):
    if not counterfactual:
        return {'valid': False, 'proximity': 0, 'sparsity': 0, 'sparsity_ratio': 0, 'constraints_satisfied': False}
    
    metrics = {
        'valid': counterfactual.get('success', False),
        'proximity': counterfactual.get('distance', 0),
        'sparsity': counterfactual.get('n_changes', 0),
        'sparsity_ratio': counterfactual.get('n_changes', 0) / max(len(counterfactual.get('original', [1])), 1)
    }
    
    if constraints and 'counterfactual' in counterfactual:
        try:
